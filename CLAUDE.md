...
**Component flow:** `App.jsx` → fixed `Navbar` + sequential sections: `Hero → About → Stack → Projects → Contact` + `Footer`. Each section is a standalone component in `src/components/`.
...
- **Accent (green):** `text-accent` `bg-accent` `border-accent` `bg-accent-dim`
- **Severity:** `text-critical` `text-high` `text-medium` `text-info` (and matching `border-l-*`)
- **Fonts:** `font-display` (Barlow Condensed — headings only, uppercase), `font-sans` (Inter — body), `font-mono` (JetBrains Mono — actual code/commands only, never prose)
